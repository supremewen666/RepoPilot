...
from repopilot.config import load_environment
from repopilot.schemas import SourceItem
...
try:
    ...
    from repopilot.compat import BaseTool
...
def build_github_server_config() -> dict[str, dict[str, Any]]:
    ...
    command = os.getenv("REPOPILOT_GITHUB_MCP_COMMAND")
    url = os.getenv("REPOPILOT_GITHUB_MCP_URL")
    args = _parse_command_args(os.getenv("REPOPILOT_GITHUB_MCP_ARGS"))
    env = _parse_json_mapping(os.getenv("REPOPILOT_GITHUB_MCP_ENV"))
    headers = _parse_json_mapping(os.getenv("REPOPILOT_GITHUB_MCP_HEADERS"))
